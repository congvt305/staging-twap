define([
    ...
    $.widget('magepow.ajaxcart', {
        ...
        _updateWishlistIcons: function (wishlist) {
            var wishlistItems = wishlist && wishlist['items'] ? wishlist['items'] : {}
            var productIds = wishlistItems.map(product => product.product_id).filter((id, index, self) => self.indexOf(id) === index).map(str => Number(str));
            $(`.action.towishlist`).each(function () {
                var productId = $(this).data('product-id');
                if (productIds.includes(productId)) {

                    $(this).addClass('wishlisticon2');
                } else {
                    $(this).removeClass('wishlisticon2');
                }
            });
        }
