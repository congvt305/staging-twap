...
define([
        ...
        _create: function () {
            ...
            this.element.on('click', '[data-wishlist-create]', function () {
                var json = $(this).data('wishlist-create'),
                    ...
                    isAjax = json.ajax,
                    popupLength = $('#' + _this.options.createTmplData.popupWishlistBlockId).length;

                if (!(i == 0 && popupLength == 1)) {
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _create: function () {
            ...
            this.element.on('click', 'li:has([data-wishlist-move-selected])', $.proxy(function (e) {
                var popupLength = $('#'+_this.options.createTmplData.popupWishlistBlockId).length;
                if (!(i == 0 && popupLength == 1)) {
                    this._moveSelectedTo(e);
                }

            }, this));

            this.element.on('click', 'li:has([data-wishlist-move-item])', $.proxy(function (e) {
                var popupLength = $('#'+_this.options.createTmplData.popupWishlistBlockId).length;
        ...
        _moveSelectedTo: function (e) {
            var json = $(e.currentTarget).find('span').data('wishlist-move-selected'),
                ...
                url,
                popupLength = $('.modal-popup.confirm').length,
                popupoOpenLendth = 0;

            if (json['new']) {
                this._moveSelectedToNew();
            } else if (!this._itemsSelected()) {
                if (!(popupLength == 1 && popupoOpenLendth == 0)) {
                    alert({
                        content: $.mage.__('Please select items to move.')
                    });
                    popupoOpenLendth = $('.modal-popup.confirm').length;
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _create: function () {
            ...
            this.element.on('click', 'li:has([data-wishlist-copy-selected])', $.proxy(function (e) {
                var popupLength = $('#'+_this.options.createTmplData.popupWishlistBlockId).length;

                if (!(i == 0 && popupLength == 1)) {
                    this._copySelectedTo(e);
                }

            }, this));

            this.element.on('click', 'li:has([data-wishlist-copy-item])', $.proxy(function (e) {
                var popupLength = $('#'+_this.options.createTmplData.popupWishlistBlockId).length;
                if (!(i == 0 && popupLength == 1)) {
                    this._copyItemTo(e);
                }
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _deleteWishlist: function (e) {
            ...
            var deleteUrl = json.deleteUrl.replace(encodeURI('%item%'), json.wishlistId);
            var redirectUrl = json.redirectUrl;

            e.preventDefault();
            var popupLength = $('.modal-popup.confirm').length,
                popupOpenLength = 0;

            if (!(popupLength == 1 && popupOpenLength == 0)) {
                confirm({
                ...
                popupOpenLength = $('.modal-popup.confirm').length;
            }
        }
    });
    ...
    $.widget('mage.multipleWishlist', $.mage.multipleWishlist, {
        ...
        _createEditWishlistModal: function (e) {
            var json = $(e.currentTarget).data('wishlist-edit'),
                self = this,
                popupLength = $('#'+self.options.editTmplData.popupWishlistFormId).length,
                popupOpenLength = 0;
            ...
            if (popupLength != 1) {
                ...
                popupOpenLength = $('#'+self.options.editTmplData.popupWishlistFormId).length;
            }
