...
class Topmenu extends \Magento\Framework\View\Element\Template
{
    ...
    public function __construct(
        ...
        \Satp\Megamenu\Helper\Data $helper,
    ...
    public function getMegamenuHtml()
    {
        ...
        $this->_megamenuConfig = $this->_helper->getConfig('satp_megamenu');
        ...
        foreach($categories as $category) {
            ...
            $satp_menu_cat_columns = $cat_model->getData('satp_menu_cat_columns');
            $satp_menu_float_type = $cat_model->getData('satp_menu_float_type');

            if(!$satp_menu_cat_columns){
                $satp_menu_cat_columns = 1;
            }
            ...
            $menu_right_content = $cat_model->getData('satp_menu_block_right_content');
            $menu_right_width = $cat_model->getData('satp_menu_block_right_width');
            if(!$menu_right_content || !$menu_right_width)
                $menu_right_width = 0;
            if($satp_menu_float_type)
                $satp_menu_float_type = 'fl-'.$satp_menu_float_type.' ';
            if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_right_content)))
                $item_class .= 'parent ';
            $html .= '<li class="ui-menu-item '.$item_class.$satp_menu_float_type.'">';
            if(count($children) > 0) {
                $html .= '<div class="open-children-toggle"></div>';
            }
            ...
            if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_right_content))) {
                ...
                if(count($children) > 0 || (($menu_type=="fullwidth" || $menu_type=="staticwidth") && ($menu_right_content))) {
                    $html .= '<div class="row">';
                    $html .= $this->getSubmenuItemsHtml($children, 1, $max_level, 12-$menu_right_width, $menu_type, $satp_menu_cat_columns);
