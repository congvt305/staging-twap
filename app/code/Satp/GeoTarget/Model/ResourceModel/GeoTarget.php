<?php
namespace Satp\GeoTarget\Model\ResourceModel;

class GeoTarget extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    protected function _construct()
    {
        $this->_init('satp_geo_target','entity_id');
    }
    ...
    protected function _afterDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if(!empty($entityId)) {
            $this->getConnection()->delete($this->getTable('satp_geo_target_store'),
                ['link_id = ?' => $entityId]
            );
        }
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if ($storeIds && ($storeIds != $object->getOrigData('store_ids'))) {
            ...
            $this->getConnection()->insertOnDuplicate($this->getTable('satp_geo_target_store'), $dataForInsert, []);
        }
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        if ($object->getEntityId()) {
            $select = $this->getConnection()->select()
                ->from($this->getTable('satp_geo_target_store'), 'store_id')
