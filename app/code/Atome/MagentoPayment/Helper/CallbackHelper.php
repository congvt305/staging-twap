...
class CallbackHelper extends \Magento\Framework\App\Helper\AbstractHelper
{
     ...
     * @throws \Exception
     */
    public function applyQuotePayment($quoteId, $queryParams)
    {
        $connection = $this->resourceConnection->getConnection('sales');
        $connection->beginTransaction();
        try {
            // acquire update lock first
            $quotes = $connection->select()->from($this->quoteRepository->getMainTable())->where('entity_id = ?', $quoteId)->forUpdate(true)->query()->fetchAll();
            if (count($quotes) !== 1) {
                throw new \RuntimeException("no quote $quoteId in database");
            }

            $ctx = $this->applyQuotePaymentInternal($quoteId, $queryParams);
            $connection->commit();
            ...
            $this->commonHelper->debug("applyQuotePayment debug trace: " . json_encode($e->getTrace()));
            $this->commonHelper->logError("applyQuotePayment failed: " . get_class($e) . ', message: ' . $e->getMessage() . ', code: ' . $e->getCode() . ', file: ' . $e->getFile() . ', line: ' . $e->getLine());
            $connection->rollBack();

            // if we have an order (and $ctx), then the transaction exception seems to be caused by transaction deadlock
            // then we can just ignore the exception safely, and continue to return the $ctx here
            $ctx = $this->initQuotePaymentContext($quoteId, $queryParams);
            $order = $this->getOrderByIncrementId($ctx->quote->getReservedOrderId());
            if (!$order) {
                throw $e;
            }
        }
     ...
     * @return bool
     */
    public function hasInvoice($order, $transactionId)
    {
