...
class Logger{
	...
	const PATH_RELATIVE_ROOT = '/var/log/eghl_Logs/'; //Path relative to root
	...
	private function __construct($order_number){
		...
		try{
			...
			if(self::$eghl_config['debug']==1){
				self::$fname = self::$OrderNumber.'.log';
				self::$fhandle = fopen(self::$LogPath.self::$fname, "a+");
				self::writeArray(self::$eghl_config, 'eGHL plugin config');
			}
		}
		catch(\Exception $e) {
			self::$magento_logger->critical("Eghl\PaymentMethod\Classes\Logger Exception: unable to Open file [".self::$fname."] -> ".$e->getMessage());
		}
	}
	...
	public static function writeString($string){
		try{
			if(self::$eghl_config['debug']==1){
				...
				while(!$written){
					...
					if(($timerNow-$timerStart) > 10){
						self::$magento_logger->critical("Eghl\PaymentMethod\Classes\Logger (writeString): -> Could not Acquire lock");
						break;
					}
			...
			self::$magento_logger->critical("Eghl\PaymentMethod\Classes\Logger Exception (writeString): -> ".$e->getMessage());
		}	
	}
		...
		try{
			if(self::$eghl_config['debug']==1){
				...
				while(!$written){
					...
					// will not wait for lock more than 10 secs
					if(($timerNow-$timerStart) > 10){
						self::$magento_logger->critical("Eghl\PaymentMethod\Classes\Logger (writeArray): -> Could not Acquire lock");
						break;
					}
					if(flock(self::$fhandle, LOCK_EX)){ // acquire an exclusive lock
			...
			self::$magento_logger->critical("Eghl\PaymentMethod\Classes\Logger Exception (writeArray): -> ".$e->getMessage());
		}	
	}
	
	// The singleton method
    public static function init($order_number)
    {
        if (!isset(self::$instance)) {
            self::$instance = new Logger($order_number);
        }
