...
class ResponseHandler extends Action
{
    const EGHL_SERVICE_ID    = 'eghl_service_id';
    const EGHL_CURRENCY_CODE = 'eghl_currency_code';
    const EGHL_PAYMENT_ID    = 'eghl_payment_id';
    const EGHL_PAYMENT_CODE  = 'eghl_payment_code';
    ...
    protected function createOrderInvoice($paymentData = [])
    {
        ...
        if ($canInvoice && !$this->_order->hasInvoices()) {
            ...
            if (isset($paymentData['TxnID']) && !empty($paymentData['TxnID'])) {
                $invoice->setTransactionId($paymentData['TxnID']);
            }
            $invoice->save();
    ...
    protected function createTransaction($order = null, $paymentData = [])
    {
        ...
        $payment->setAdditionalInformation(
            [
                ...
                self::EGHL_CURRENCY_CODE => $paymentData['CurrencyCode'],
                self::EGHL_PAYMENT_CODE  => $paymentData['PymtMethod'],
                self::EGHL_PAYMENT_ID    => $paymentData['PaymentID'],
                self::EGHL_SERVICE_ID    => $paymentData['ServiceID'],
            ]
        );
                    ...
                    if ($vars['CurrencyCode']==$this->_order->getOrderCurrencyCode() && $vars['Amount']!=$this->_order->getGrandTotal()) {
                        ...
                        if ($vars['TransactionType']=="SALE") {
                            ...
                            if (strcasecmp($hash2, $vars['HashValue2'])!=0) { //Different hash between what we calculate and the hash sent by the payment platform so we do not do anything as we consider that the notification doesn't come from the payment platform.
                                ...
                                // Detect first callabck footstep; if found then exit the code
