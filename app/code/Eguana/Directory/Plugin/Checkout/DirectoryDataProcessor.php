...
class DirectoryDataProcessor
{
    ...
    public function afterProcess(\Magento\Checkout\Block\Checkout\DirectoryDataProcessor $subject, $result, $jsLayout)
    {
        ...
        foreach ($result['components']['checkoutProvider']['dictionaries']['region_id'] as $key => $value){
            if ($key == 0) {
                continue;
            }
            $result['components']['checkoutProvider']['dictionaries']['region_id'][$key]['label'] = __($value['label']);
        }
        foreach ($result['components']['checkoutProvider']['dictionaries']['region_id'] as $key => $value){
        ...
        foreach ($result['components']['checkoutProvider']['dictionaries']['city_id'] as $key => $value){
            if (empty($value['value'])) {
                continue;
            }
            $result['components']['checkoutProvider']['dictionaries']['city_id'][$key]['label'] = __($value['label']);
        }
        return $result;
    }
