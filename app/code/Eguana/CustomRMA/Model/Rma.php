    ...
    private function getBundleChilds($item, $orderItemId)
    {
        ...
        foreach ($this->getOrder()->getItems() as $orderItem) {
            if ($orderItem->getParentItemId() == $orderItemId) {
                $itemModel = $this->_rmaItemFactory->create();
                $itemChild['order_item_id'] = $orderItem->getItemId();
                $itemChild['qty_requested'] = ($itemModel->getReturnableQty($this->getOrder()->getId(), $orderItem->getItemId()) / $itemParentQtyReturn) * $item['qty_requested'];
