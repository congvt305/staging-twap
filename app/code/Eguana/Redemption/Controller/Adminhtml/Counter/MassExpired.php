...
/**
 * use that class for Mass Expired action
 *
 * Class MassExpired
 */
class MassExpired extends Action implements HttpPostActionInterface
{
    /**
    ...
    public function execute()
    {
        try {
            ...
            foreach ($collection as $item) {
                $item->setStatus(Counter::STATUS_EXPIRED);
                $item->save();
            }
            $this->messageManager->addSuccessMessage(
                __('A total of %1 record(s) have been expired.', $collection->getSize())
            );
