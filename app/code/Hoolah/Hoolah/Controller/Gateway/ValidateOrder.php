<?php
    ...
    class ValidateOrder extends \Magento\Framework\App\Action\Action
    {
        // protected
        ...
        // /vendor/magento/framework/Webapi/ServiceInputProcessor.php (_createFromArray)
        protected function updateAddress($object, $data)
        {
            foreach ($data as $propertyName => $setterValue) {
                ...
                if (in_array($setterName, array('setQuote', 'setExtensionAttributes'))) // ignore it
                    continue;
        ...
        public function execute()
        {
            ...
            try
            {
                ...
                foreach ($clear_country_codes as $dial_code => $country_code)
                    if (substr($phone_cleared, 0, strlen($dial_code)) == $dial_code)
                    {
                        $code = $dial_code;
