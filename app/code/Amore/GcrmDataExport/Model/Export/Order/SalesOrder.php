...
/**
 * This class Exports Sales Order
 *
 * Class SalesOrder
 */
class SalesOrder extends AbstractEntity implements OrderColumnsInterface
{
        ...
        foreach ($ordersData as $orders) {
            foreach ($orders as $singleOrder) {
                if ($index == 0) {
                    ...
                    $data = array_intersect_key($singleOrder, $this->getArrayValue(SalesOrder::HEAD_COLUMNS_NAME));
                    $writer->setHeaderCols(array_keys($data));
                    $index += 1;
                }
     ...
     * @return array
     */
    private function getArrayValue(array $originCols)
    {
        $originColumn = [];
        if ($originCols) {
